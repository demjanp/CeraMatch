...
class Clustering(DModule):
	...
	def __init__(self, model):
		...
		self._data = [{}, set([]), set([]), {}, {}]  # clusters, nodes, edges, labels, positions
	...
	def load(self):
		...
		clusters, nodes, edges, labels, positions = self.format_node_ids(clusters, nodes, edges, labels, positions)
		
		self.set_data(clusters, nodes, edges, labels, positions)
	...
	def delete(self):
		...
		for cls in [self.model.cluster_class, self.model.node_class]:
			self.view.progress.update_state(text = "Deleting...", value = cnt, maximum = 3)
			del self.model.classes[cls]
			cnt += 1
		self.view.progress.update_state(text = "Deleting...", value = 3, maximum = 3)
	...
	def make(self, max_clusters, limit):
		...
		clusters, nodes, edges, labels = get_clusters(self.model.distance, max_clusters = max_clusters, limit = limit, progress = self.view.progress)
		if not clusters:
			return None, None, None, None
	...
	def update(self):
		...
		if clusters:
			...
			clusters, nodes, edges, labels = update_clusters(self.model.distance, clusters, progress = self.view.progress)
			if not clusters:
				return None, None, None, None
			self._has_clusters = True
			clusters, nodes, edges, labels = self.format_node_ids(clusters, nodes, edges, labels)
			...
			self._has_clusters = False
			clusters, nodes, edges, labels = get_clusters(self.model.distance, max_clusters = len(self.model.sample_ids), progress = self.view.progress)
			if not clusters:
				return None, None, None, None
	...
	def add_cluster(self, cluster_id, node_ids):
		...
		for node_id in node_ids:
			self._data[2].add((cluster_id, node_id))
	...
	def remove(self, node_id):
		...
		if node_id in self._data[0]:
			del self._data[0][node_id]
		if node_id in self._data[1]:
			self._data[1].remove(node_id)
		edges_new = set([])
		for node_id1, node_id2 in self._data[2]:
			if (node_id == node_id1) or (node_id == node_id2):
				continue
			edges_new.add((node_id1, node_id2))
		self._data[2] = edges_new
		del self._data[3][node_id]
		if node_id in self._data[4]:
			del self._data[4][node_id]
	...
	def add_child(self, cluster_id, node_id):
		...
		self._data[0][cluster_id].append(node_id)
		self._data[2].add((cluster_id, node_id))
	...
	def remove_child(self, cluster_id, node_id):
		...
		self._data[0][cluster_id].remove(node_id)
		self._data[2].remove((cluster_id, node_id))
	...
	def get_data(self):
		...
		return [copy(item) for item in self._data]
	
	def set_data(self, clusters = None, nodes = None, edges = None, labels = None, positions = None):
		
		if isinstance(nodes, list):
			nodes = set(nodes)
		if isinstance(edges, list):
			edges = set(edges)
		for i, item in enumerate([clusters, nodes, edges, labels, positions]):
			if item is not None:
	...
	def import_cluster_data(self, data):
	...
	def import_xlsx(self, path, sample_column, cluster_column):
		...
		return self.import_cluster_data(data)
